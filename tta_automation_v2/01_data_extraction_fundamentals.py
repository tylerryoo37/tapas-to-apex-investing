...
if __name__ == "__main__":
    ...
    # Option 3: Use PRE-FILTERED tickers from preprocessing script (RECOMMENDED!)
    # Run ../00_ticker_preprocessing.py first to generate filtered ticker files
    # This uses tickers that already passed market cap, price, and volume filters

    # Use the most recent filtered ticker file (all filtered tickers)
    # ticker_type = 'filtered_all'
    # filtered_date = '20250107'  # Update this to match your filtered file date
    # with open(f'../filtered_tickers_{filtered_date}.txt', 'r') as f:
    #     my_tickers = [line.strip() for line in f if line.strip()]

    # OR use a specific segment (500 tickers each - much faster!)
    # ticker_type = 'filtered_segment_0'
    # filtered_date = '20250107'  # Update this to match your filtered file date
    # segment_num = 0  # Change this to process different segments (0, 1, 2, ...)
    # with open(f'../filtered_tickers_{filtered_date}_segment_{segment_num}.txt', 'r') as f:
    #     my_tickers = [line.strip() for line in f if line.strip()]

    # Option 4: Use unfiltered tickers (NOT RECOMMENDED - too many API calls)
